const apikey = '604400e1-7a02-4ddc-bae5-9f4ebcd92708';
const apihost = 'https://todo-api.coderslab.pl';

if (apikey == '') {
    alert('Go to https://todo-api.coderslab.pl/apikey/create and copy the generated token and paste it into the const "apikey", in the script.js file. Otherwise it will not work!');
}
...
function apiListAllTasks() {
    return fetch(
        ...
        function (resp) {
            if (!resp.ok) {
                alert('An error occurred! Open devtools and the Network tab, and look for the cause');
            }
            return resp.json();
        }
    );
...
function renderTask(taskId, title, description, status) {
    ...
    section.style = 'margin-top: 30px;box-shadow: 5px 10px #130';
    document.querySelector('main').appendChild(section);

    const mainDiv = document.querySelector('main div');
    mainDiv.style = 'background-color: #EFFEC7; box-shadow: 5px 10px #130';
    mainDiv.classList.remove('card', 'shadow');

    const headerDiv = document.createElement('div');
    headerDiv.className = 'card-header d-flex justify-content-between align-items-center';
    headerDiv.style = 'background-color: #EFFEC7';
    ...
    if (status == 'open') {
        ...
        descriptionInput.setAttribute('minlength', '10');
    ...
    // lastButton.style = "box-shadow: 3px 7px #130";
}
...
function renderOperation(ul, status, operationId, operationDescription, timeSpent) {
    ...
    if (status == "open") {
        ...
        const add2hButton = document.createElement('button');
        add2hButton.className = 'btn btn-outline-success btn-sm mr-2';
        add2hButton.innerText = '+2h';
        controlDiv.appendChild(add2hButton);

        add2hButton.addEventListener('click', function () {
            apiUpdateOperation(operationId, operationDescription, timeSpent + 120).then(
                function (response) {
                    time.innerText = formatTime(response.data.timeSpent);
                    timeSpent = response.data.timeSpent;
                }
        ...
        resetButton.style = "margin-right: 7px"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.body.style.backgroundColor = "#1A5E25";
});
